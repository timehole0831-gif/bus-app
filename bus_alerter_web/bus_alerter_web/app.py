...
def check_buses_and_notify():
    ...
    try:
        ...
        print(f"DB 오류 (아직 테이블이 생성되지 않았을 수 있음): {e}")
        conn.close()
        return # DB가 준비되지 않았으면 작업 종료
...
# (중요!) Gunicorn이 import할 때 이 코드가 실행되어야 합니다.
init_db() # 1. 앱 시작 시 DB 테이블이 없으면 생성
print("--- 데이터베이스 초기화(init_db) 완료 ---")
...
scheduler.start()
print("--- 백그라운드 스케줄러(apscheduler) 시작 완료 ---")
...
if __name__ == "__main__":
    # 4. (로컬 테스트용) Flask 앱 실행
    # Gunicorn은 이 부분을 실행하지 않습니다.
    # use_reloader=False 는 스케줄러가 두 번 실행되는 것을 방지하기 위해 필수
    print("--- 로컬 테스트 서버(app.run) 시작 ---")
